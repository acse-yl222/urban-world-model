"""23 Queen’s Gate Mews: mapped two-storey house, licensed streetscape-informed artistic detail.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min((1.22 if floor==0 else 1.38) if ei==entry_edge else 1.24,L/count-.7);bot=low+(.88 if floor==0 else .92);top=high-.60
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'artistic mews house arrangement; exact no.23 not identified in licensed views'})
    ...
    mesh('complete six-point roof deck','roof',v,f)
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='23 Queen’s Gate Mews | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='OSM-addressed two-storey house; exact-photo identification unresolved; streetscape-informed artistic detail';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.31,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-809386116','qgm2014','rbkc-pp1002541'],'uncertainty':['OSM address and two levels retained; inherited 6.850666911m eaves is a formula estimate, not survey','Two licensed photographs actually inspected; qgm2014 is streetscape context only, qgm2013 ornate crop excluded from target derivation; muted masonry, opening layout and flat roof are artistic','Exact-address planning PP/10/02541 is permission history, not asbuilt; no basement, linking extension or garage inferred','Complete six-point roof and walls retained; all three normalized shared segments kept opaque at supplied height intervals, against mapped22,24 and4 Queens Gate','Original western entrance lateral position and base retained; external ground and approximately49.558mm ground-to-door difference belong to coordinator']}
