"""48 Princes Gardens: official group identity, licensed terrace-context details.
Metric heights, hidden elevations and flat roof detail remain explicit estimates.
"""
import math
...
def build(feature, materials):
    ...
    for seg in feature['audit']['normalized_shared_wall_segments']:
        ...
        source_interval=seg.get('height_interval')
        zz=[max(z0,source_interval[0]),min(z0+H,source_interval[1])] if source_interval else [z0,z0+H]
        ...
        common.append({'edge':ei,'s_interval':[max(0,span[0]),min(L,span[1])],'height_interval':zz,'polyline':line,'neighbour':seg['neighbour'],'openings':[],
                       'source_neighbour_height_interval':source_interval,
                       'height_interval_semantics':'mapped common interval' if source_interval else 'full target aperture exclusion only; neighbour height remains unknown'})
    ...
    # Three front bays are a labelled terrace-context estimate, not a parcel photo measurement.
    for ei,(a,L,u,n) in enumerate(edges):
        if ei==0:positions=[L*(i+.5)/3 for i in range(3)]
        elif ei in[1,2,4]:positions=[]  # Narrow inset return cheeks stay solid.
        else:
        ...
        for floor,(lo,hi)in enumerate(zip(floors,floors[1:])):
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                panel(ei,'entrance head frame'if door else'timber sash upper rail','trim',l+.056,r-.056,top-.055,top,-.10,.12)
                if door:
                    ...
                    for x in[l-.10,r+.10]:panel(ei,'entrance stone surrounds','trim',x-.07,x+.07,bot,top,.04,.18)
                    panel(ei,'entrance stone head','trim',l-.17,r+.17,top,top+.14,.04,.18)
                    entrance={'threshold_xyz':[*E['center_xy'],E['threshold_z']],'outward_normal':[*n,0.0],'door_leaf_xyz':list(at(ei,mid,-.35,(bot+top)/2)),'clear_width_m':E['clear_width_m'],'clear_height_m':top-bot,'leaf_recess_m':.35,'stair_treads':[],'ramp':'none authored','surface_owner':'coordinator','additional_supports':[],'basis':'Original threshold and tangent position retained; finite approach from audited source ground-.05 belongs to coordinator'}
                else:
                    for zz in[bot+.03,(bot+top)/2]:panel(ei,'sash meeting and bottom rails','trim',l+.056,r-.056,zz-.025,zz+.025,-.10,.11)
                    if floor==1:panel(ei,'first floor French casement mullion','trim',mid-.025,mid+.025,bot+.055,top-.055,-.085,.11)
                    panel(ei,'projecting stone sill','trim',l-.12,r+.12,bot-.11,bot+.01,.06,.30)
                    for x in[l-.06,r+.06]:panel(ei,'architrave upright','trim',x-.045,x+.045,bot+.01,top+.02,.015,.13)
    ...
    for z,h in[(floors[1]-.1,.16),(floors[2]-.09,.12),(H-.34,.16),(H-.12,.105)]:band('continuous mitred contained stringcourse','trim',z,z+h,-.26,0)
    panel(0,'front bracketed frieze','trim',.18,edges[0][1]-.18,H-.43,H-.16,.09,.26)
    panel(0,'front projecting cornice','trim',.18,edges[0][1]-.18,H-.17,H-.015,.15,.40)
    for j in range(19):
        ss=.4+j*(edges[0][1]-.8)/18;panel(0,'cornice modillions','trim',ss-.06,ss+.06,H-.44,H-.18,.16,.32)
    cap('complete concave mapped roof deck',ring,H+.008)
    # No direct target roof view: retain flat source deck and low front/rear parapets.
    # Shared side walls get no raised coping that might obstruct neighbours.
    for ei in[0,3]:
        ll=edges[ei][1]
        panel(ei,'estimated front rear roof parapet','masonry',.28,ll-.28,H,H+.34,-.13,.24)
        panel(ei,'front rear coping','trim',.25,ll-.25,H+.34,H+.44,-.13,.30)
    F=floors[1];L=edges[0][1]
    panel(0,'continuous first floor balcony deck','trim',.30,L-.30,F-.10,F+.07,.44,1.12)
    for j in range(25):column('first floor bottle balusters',at(0,.47+j*(L-.94)/24,.92,0)[:2],F+.07,F+.78,.045)
    panel(0,'continuous first floor balustrade rail','trim',.30,L-.30,F+.78,F+.90,.92,.21)
    # The preserved source entry is rear-facing edge3. No front-photo Doric porch
    # is transferred to it, and no unverified new north-front door is cut.
    if entrance is None:raise ValueError('Missing original rear entrance')
    entrance['additional_supports']=[]
    created=[]
    for (g,m),(vv,ff)in groups.items():
        if not ff:continue
        used=sorted({i for face in ff for i in face});remap={old:new for new,old in enumerate(used)}
        vv=[vv[i] for i in used];ff=[tuple(remap[i] for i in face) for face in ff]
        name='48 Princes Gardens | '+g;me=bpy.data.meshes.new(name);me.from_pydata(vv,[],ff);me.update()
        bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();me.update()
        ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);me.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Official five-storey group; CC BY-SA2 terrace context, exact48 details and metric heights estimated';created.append(ob.name)
    return {'created':created,'parameters':{'base_z':z0,'main_wall_height_m':H,'height_m':H,'eaves_height_m':H,'levels':5,'roof_levels':0,'roof_max_z':z0+H+.44,'footprint_area_m2':area,'wall_thickness_m':.44,'opening_count':len(openings)},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':[{'name':'full source footprint flat roof','z':z0+H+.008,'surface_separation_m':.008},{'name':'estimated front rear parapet','max_z':z0+H+.44}], 'evidence_source_ids':['osm-640808105','commons-166589109','westminster-2018-48pg'],'observations':['Official2018Westminster report establishes46–48PrincesGardens group five floors above basement and rear alterations','Licensed close terrace image shows pale stucco, Doric porches, Corinthian-style upper details and balustrades; exact48image-to-bay assignment remains unresolved'],'uncertainty':['Main height16.29974826m and front/rear parapet.44m are estimates, not survey','Three front bays, decorative details and rear openings are estimated from group context, not confirmed same-building photo','Flat roof retained from source; historical group plant-area text does not establish specific48equipment or location, so none invented','Original rear-facing edge3 nearzero entry retained with coordinator finite approach from sourceground-.05 to road0; no copied front porch, stairs, new front door or basement excavation','Both normalized segments retained; unknown west neighbour height stays null in source metadata while aperture exclusion spans full target height','No Blender integration or visual validation claimed by author']}
