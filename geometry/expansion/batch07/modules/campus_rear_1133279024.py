"""Campus rear 1133279024: mapped anonymous service wing between ACEX and Sherfield; artistically completed elevations.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min(1.55,L/count-.85);bot=low+1.05;top=high-.62
    ...
    def band(g,m,lo,hi,dep,th):
        ...
        mesh(g,m,v,f)
    for zz,th,dep in [(H-.12,.17,.02)]:band('plain eaves and floor course','trim',zz-th/2,zz+th/2,dep,.22)
    band('service wing parapet','masonry',H,H+.37,-.17,.34);band('stone parapet coping','trim',H+.37,H+.47,-.15,.4)
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='Campus rear 1133279024 | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped anonymous campus rear footprint; all elevations artistically completed; ACEX photo context only';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.47,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-1133279024','commons-acex-callendar-2020'],'uncertainty':['Anonymous OSM building; campus rear is a working label and no institutional function is asserted','Four storeys inferred from procedural source height, not observed; original 13.1505217555m eaves preserved','Viewed licensed ACEX photo shows neighbouring grey panelled facade and horizontal glazing; exact target is not identifiable in that image; target masonry, casements and flat roof are artistic completion','ACEX and Sherfield neighbour eaves unresolved: shared-boundary walls retained complete and conservatively unperforated across target height; this is not measured common-wall height','No unobserved roof plant or institutional signage added']}
