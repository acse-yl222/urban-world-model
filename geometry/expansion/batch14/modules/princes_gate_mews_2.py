"""2 Princes Gate Mews OSM-ID-specific two-storey interpretation.
...
def build(feature, materials):
    ...
    # Four narrow upper front sashes and two broader ground casements flank
    # the retained entry; the unobserved rear has a quieter two-bay treatment.
    floor_positions={4:{0:[2.20,7.85],1:[1.48,3.82,6.20,8.56]},
                     2:{0:[1.75,4.98],1:[1.90,4.84]}}
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(floor_positions.get(ei,{}).get(floor,[])):
                ...
                w=(2.00 if ei==4 else 1.35) if floor==0 else 1.18
    ...
    def band(g,m,lo,hi,inner,outer,free_only=False):
        ...
        mesh(g,m,v,faces)
    # Insets keep all mitred roof trim inside the mapped plan. No parapet rises
    # on the three shared edges; low frontage/rear strips have capped ends.
    ...
    band('front and rear low parapet','masonry',H-.01,H+.22,-.25,-.005,True)
    band('front and rear contained coping','trim',H+.22,H+.30,-.29,-.002,True)
    vv=[Vector((x,y,z0+H+.008)) for x,y in ring]
    for tri in tessellate_polygon([vv]):
        mesh('complete five point roof cap','roof',[tuple(vv[q] if isinstance(q,int) else q) for q in tri],[(0,1,2)])
    roof_parts.append({'name':'complete mapped five point flat roof','z':z0+H+.008,
                       'max_z':z0+H+.30,'parapet_edges':[2,4],
                       'shared_edges_without_raised_parapet':[0,1,3],
                       'basis':'Source flat typology retained, detailed shape estimated; cap lifted8mm to avoid coplanar wall faces'})
    created=[]
    for (group,mat),(vertices,faces) in groups.items():
        ...
        name='2 Princes Gate Mews | '+group;m=bpy.data.meshes.new(name);m.from_pydata(vertices,[],faces);m.update()
    ...
    return {'created':created,'parameters':{'base_z':z0,'height_m':H,'main_wall_height_m':H,'eaves_height_m':H,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.30,'wall_thickness_m':.40,'opening_count':len(openings)},
            'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,
            'evidence_source_ids':['osm-851362836','geograph-7709889-context'],
            'observations':['Licensed street view shows stock-brick walls, pale divided windows, simple eaves and dark carriage doors; exact2 is not identified'],
            'uncertainty':['Facade bays, window dimensions, rear apertures, flat roof detail and rainwater pipe are artistic completion, not mapped observations',
                           'Two levels are OSM tags;6.849684m is the retained source estimate, not a measured eaves height',
                           'All three normalized shared segments retain walls and suppress overlapping openings; no neighbour walls are removed',
                           'No inferred garage, basement, mansard, extra entry or imported1PrincesGateMews ramp',
