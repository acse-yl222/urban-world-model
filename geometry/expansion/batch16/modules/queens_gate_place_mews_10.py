"""10 Queens Gate Place Mews: full mapped ring; Residential mews from OSM; individual facade estimated.
...
def build(feature, materials):
    ...
    materials=dict(materials)  # original warm masonry palette; exact no10 colour remains artistic
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.48 if ei==1 else 1.05,L/count-.7)
    ...
    # Estimated modest hipped roof entirely inside plan; no dormer inferred from other houses.
    ...
    vs=[(x,y,z0+H+.03) for x,y in rr]+[(r0[0],r0[1],z0+H+.85),(r1[0],r1[1],z0+H+.85)]
    meshpart('estimated hipped roof','roof',vs,[(0,1,5,4),(1,2,5),(2,3,4,5),(3,0,4)])
    created=[]
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='10 Queens Gate Place Mews | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
    ...
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.85},'interfaces':{'entrance':entrance,'additional_entrances':[],'additional_supports':[],'shared_walls':shared,'shared_wall_policy':'Every normalized common-height segment retained blind; full footprint preserved'},'openings':openings, 'evidence_source_ids':['osm-810633523','mews2010'],'uncertainty':['OSM identifies no10 as house, not neighbouring no14 motor showroom; exact present use not independently confirmed','No licensed photo securely identifies no10; warm masonry, paired casements and dentils are street-context artistic completion','OSM two levels and original estimated eaves retained; no evidence justifies adding other houses dormer storeys','Complete original plan and internal hip roof85cm rise retained; rear windows and roof are unmeasured artistic details','All normalized shared-wall segments retained blind; only original offset east rear door interface, no invented garage entrance','Source ground-.05 and original threshold retained; coordinator owns finite support']}
