"""Jay Mews 117010283: full mapped ring; individually completed terraced mews exterior.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=max(1,int(L/3.1)) if L>2.5 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(2.10 if level==0 else 1.20,L/count-.8)
    ...
    for zz in floor_bounds[1:-1]:band('projecting storey ledge','trim',zz-.06,zz+.08,.035,.13)
    ...
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='Jay Mews 117010283 | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
