...
const counts=Object.fromEntries([...wanted].map(id=>[id,objects.filter(o=>o.id===id).length]));const missing=Object.entries(counts).filter(([,n])=>!n).map(([id])=>id);if(missing.some(id=>!['way-359620473','way-809238790'].includes(id)))throw Error('Unexpected missing required assets '+missing.join(','));
const basis={method:'Decoded original source plus latest delivered replacement overlays in chronological report order; preserved original entry support meshes',inputs,replacements,required_building_object_counts:counts,missing_ids:missing,missing_context_policy:"way-359620473 historically absent; way-809238790 audited absent in original and supplement GLB node metadata, asset index and source-part aliases in neighbour_809238790_asset_audit.json; retain mapped neighbour with unknown height, no surrogate"};
