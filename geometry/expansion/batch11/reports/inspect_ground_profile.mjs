...
const manifest=JSON.parse(fs.readFileSync('geometry/expansion/batch11/manifest.json'));
const distancesOut=[-.25,-.12,0,.12,.2,.3,.4,.5,.6,.7,.8,.9,1,1.2]; const targets=manifest.features.flatMap(f=>[-.6,0,.6].map(l=>({id:f.id+'::lateral='+l,entry:f.entry,samples:distancesOut.map(d=>[f.entry.center_xy[0]+f.entry.outward_normal_xy[0]*d-f.entry.outward_normal_xy[1]*l,f.entry.center_xy[1]+f.entry.outward_normal_xy[1]*d+f.entry.outward_normal_xy[0]*l])})));
const results=[];
...
fs.writeFileSync('geometry/expansion/batch11/reports/ground_profile_detailed.json',JSON.stringify({source:'south_kensington_core008_web.glb',method:'Decoded original mesh; vertical XY barycentric hits at three lateral tracks,14 inward/outward distances. Bounds distances only for nearby no-hit objects, not exact mesh distances.',targets,objects:results},null,2));console.log(JSON.stringify(results.filter(o=>o.hits.length).map(o=>({name:o.name,hits:o.hits})),null,2));
