"""Jay Mews 117010293: full mapped ring; individually completed terraced mews exterior.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=max(1,int(L/3.5)) if L>2.5 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.75 if level==0 else 1.3,L/count-.8)
            ...
            for l,r in zip(cuts,cuts[1:]):
                ...
                if hole:
                    panel('pierced masonry','trim' if level==0 else 'masonry',l,r,low,hole[2]);panel('pierced masonry','trim' if level==0 else 'masonry',l,r,hole[3],high)
                else:panel('pierced masonry','trim' if level==0 else 'masonry',l,r,low,high)
    ...
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='Jay Mews 117010293 | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
        bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(mesh);bm.free()
        ob=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped footprint; inherited estimated four-floor massing; artistic terraced facade completion';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.38},'interfaces':{'entrance':entrance,'shared_walls':shared,'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-way-117010293','jay293-mews-context'],'uncertainty':['Exact way-117010293 facade not confirmed in licensed images; all bay positions and window sizes artistically completed','Current address and institutional ownership not independently confirmed; no signage modelled','Flat roof, parapet and door proportions artistically completed; existing eaves height retained exactly; no roof plant inferred','Shared-wall adjacency unknown; full walls preserved']}
