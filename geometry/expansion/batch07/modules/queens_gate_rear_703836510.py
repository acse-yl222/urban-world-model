"""Queens Gate rear703836510: full mapped ring; individually completed terraced mews exterior.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=max(1,int(L/3.15)) if L>2.5 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.6 if level==0 else 1.25,L/count-.8)
    ...
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='Queens Gate rear703836510 | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
