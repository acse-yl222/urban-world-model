"""Batch15 only: normalize exact mapped shared segments and annotate evidence limits."""
import json,sys,hashlib
from pathlib import Path
sys.path.insert(0,str(Path('geometry/completion/.deps').resolve()))
from shapely.geometry import shape,LineString
R=Path('geometry/expansion/batch15');a=json.load(open('geometry/expansion/asset_index.json'));assets={b['id']:b for b in a['buildings']};protected={bid for bid,b in assets.items() if any('source-informed' in s or 'inherited Phase4' in s for s in b.get('statuses',[]))};assert len(protected)==74
