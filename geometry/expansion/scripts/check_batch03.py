...
ROOT=Path(__file__).resolve().parents[1];RUN=ROOT/'output/batch03_v1';BATCH=ROOT/'batch03'
...
for f in features:
    ...
    for wall in interfaces.get('shared_walls',[]):
        ...
        lo,hi=wall['height_interval']
        walls=[tree for ob,tree in trees if 'pierced' in ob.name.lower()]
        assert walls
    ...
    for ix in range(math.ceil((max(xs)-min(xs))/1.25)):
        ...
        for iy in range(math.ceil((max(ys)-min(ys))/1.25)):
            ...
            samples+=1
    assert samples>20
    reports.append({'building_id':f['id'],'entrances':clearance,'shared_wall_samples':shared_samples,'roof_coverage_samples':samples,'footprint_area_m2':f['area_m2']})
