"""22 Princes Gate: mapped five-storey townhouse, exact licensed street evidence and estimated hidden roof.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        door_info=entry if ei==entry_edge else feature.get('estimated_front_entry') if ei==3 else None
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min((1.64 if floor<4 else 1.00) if ei==3 else 1.20,L/count-.8);bot=low+(.30 if floor==1 else .52);top=high-(.76 if floor==1 else .42)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'licensed sequence-inferred north facade; metric arrangement and hidden facades estimated'})
        ...
        if ei==3:
            ...
            for cs in centers:
                ...
                panel('first order window entablature','trim',cs-1.19,cs+1.19,bounds[2]-.76,bounds[2]-.56,.07,.30)
                for k in range(10):box('window entablature dentils','trim',at(cs-1.08+k*.24,.22,bounds[2]-.78),(.09,.11,.08),u,n)
            ...
            for ss in (door_s-cs_off,door_s+cs_off):
                box('Ionic porch plinth','trim',at(ss,cd,.05),(.40,.40,.10),u,n)
            ...
            supports.append({'polygon_xy':[list(at(door_s+x,d,0)[:2]) for x,d in [(-ph,door_info['support_depth_interval_m'][0]),(ph,door_info['support_depth_interval_m'][0]),(ph,door_info['support_depth_interval_m'][1]),(-ph,door_info['support_depth_interval_m'][1])]],'top_z':z0,'basis':'Estimated Ionic porch feet support; coordinator owns ground'})
            # Enriched fifth-tier panels inferred from the immediate-left-of23
            # licensed facade. Relief motifs are artistic, not traced sculpture.
            panel('plain fifth-storey front cornice','trim',.025,L-.025,H-.26,H-.13,.065,.22)
            panel('fifth tier relief lower stringcourse','trim',.025,L-.025,bounds[4]-.12,bounds[4]-.04,.06,.20)
            for cs in [L/3,2*L/3]:
                bot=bounds[4]+.48;top=H-.45;mid=(bot+top)/2
                panel('fifth tier recessed relief backing','trim',cs-.49,cs+.49,bot,top,.025,.06)
                # Segmented frame joins have no overlapping coplanar faces.
                for xx in (cs-.49,cs+.49):panel('fifth tier relief panel frames','trim',xx-.03,xx+.03,bot-.04,top+.04,.075,.08)
                for zz in (bot-.01,top+.01):panel('fifth tier relief panel frames','trim',cs-.46,cs+.46,zz-.03,zz+.03,.075,.08)
                # Low faceted central rosette and paired leaf blocks.
                N=12;rv=[at(cs+.16*math.cos(k*2*math.pi/N),d,mid+.16*math.sin(k*2*math.pi/N)) for d in (.075,.14) for k in range(N)]
                rf=[tuple(range(N-1,-1,-1)),tuple(range(N,2*N))]+[(k,(k+1)%N,(k+1)%N+N,k+N) for k in range(N)]
                mesh('fifth tier simplified relief rosettes','trim',rv,rf)
                for sign in (-1,1):
                    for zz in (mid-.19,mid+.19):box('fifth tier relief leaf accents','trim',at(cs+sign*.32,.115,zz),(.12,.08,.17),u,n)
            for fl in (2,3):
                zb=bounds[fl]+.52
                for cs in centers:
