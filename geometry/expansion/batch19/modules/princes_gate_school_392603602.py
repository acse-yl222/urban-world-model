"""23 Princes Gate (historic school): mapped five-storey townhouse, exact licensed street evidence and estimated hidden roof.
...
def build(feature, materials):
    ...
    bounds=[H*t for t in (0,.23,.48,.70,.885,1)]
    ...
    entry=dict(feature.get('entry') or {});entry.update({'porch_half_width_m':1.25,'column_outward_m':.65,'column_lateral_offset_m':.92,'porch_support_depth_interval_m':[-.12,1.10]});exy=entry.get('center_xy')
    def distance(i):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        count=(3 if ei==2 else max(1,round(L/4.0))) if L>2.8 else 0
        centers=[L*(k+.5)/count for k in range(count)]
        door_info=entry if ei==entry_edge else None
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min((1.70 if floor<4 else 1.05) if ei==2 else 1.20,L/count-.8);bot=low+(.30 if floor==1 else .52);top=high-(.76 if floor==1 else .42)
        ...
        if ei==2:
            ...
            # No23 has plain outer piers and a compact top tier under stepped
            # cornice lines; do not copy no24's pronounced corner quoins.
            panel('plain fifth-storey front cornice','trim',.03,L-.03,H-.26,H-.13,.065,.22)
            panel('short fifth tier lower stringcourse','trim',.025,L-.025,bounds[4]-.15,bounds[4]-.065,.055,.19)
            panel('short fifth tier upper cornice lip','trim',.025,L-.025,H-.115,H-.065,.10,.29)
            for fl in (2,3):
                zb=bounds[fl]+.52
                for cs in centers:
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='23 Princes Gate (historic school) | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='OSM full concave plan; HE1265482 five levels and exact licensed no23 facade; hidden geometry estimated';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.31,'opening_count':len(openings),'wall_thickness_m':.4,'floor_bounds_m':bounds,'front_edge':2},'interfaces':{'entrance':entrance,'additional_entrances':additional,'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-392603602','he-1265482','front23-25-2017'],'uncertainty':['HE1265482 and exact numbered2017photo support five storeys; metric height remains inherited estimate','Short fifth tier is included in five levels, no additional attic storey','North front Ionic porch and three-bay order follow exact23photo, narrow short fifth-tier sashes and plain outer piers; no current school signage; positions and dimensions estimated','Full four-point roof is artistically completed behind low parapet; hidden rear facades and roof equipment unverified','Both normalized common walls against22 and24 retained opaque over supplied height intervals','Original north entrance retained at source tangent, near the photo right bay; no additional door. Porch support by coordinator; no basement excavation or terrain survey claim']}
