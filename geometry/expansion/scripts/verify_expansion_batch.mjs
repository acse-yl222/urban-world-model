...
const results=[];
const allIds=EXPANSION_BATCHES.flatMap(b=>b.ids);assert(new Set(allIds).size===allIds.length,'Duplicate building ID across loaded overlays');
for(const batch of EXPANSION_BATCHES){
 ...
 assert(support.every(o=>o.visible),'Original entry supports lost');
 results.push({ids:batch.ids,sha256:createHash('sha256').update(bytes).digest('hex'),triangles:before,originals_hidden_before_batch:true,entry_supports_retained:support.length,comparison_passed:true});
}
const reportDir=process.argv[2];if(!/^geometry_expansion\/(?:batch[0-9]+|revision[0-9]+)\/reports$/.test(reportDir))throw new Error('Pass batch or revision reports directory');
fs.writeFileSync(reportDir+'/viewer_check.json',JSON.stringify({passed:true,total_buildings:results.reduce((a,b)=>a+b.ids.length,0),batches:results,scope:'Actual source GLB decode and Three.js loader/batching; no live browser screenshot'},null,2));
console.log(results);
