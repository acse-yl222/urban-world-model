"""4a Princes Gate Mews OSM-ID-specific two-storey interpretation.
...
def build(feature, materials):
    ...
    # Target-specific narrower paired front openings and larger rear casements.
    # Exact bays are artistic completion, not identification from context photo.
    floor_positions={1:{0:[1.43,3.62],1:[1.40,3.72,6.02]},
                     3:{0:[1.85,5.35],1:[1.85,5.35]}}
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(floor_positions.get(ei,{}).get(floor,[])):
                ...
                w=(1.30 if ei==1 else 1.75) if floor==0 else (1.18 if ei==1 else 1.50)
                bot=low+(.75 if floor==0 else .69);top=high-(.62 if floor==0 else .54)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                if not door:
                    ...
                    x=l+w*.50
                    panel('paired narrow casement central stile','metal',x-.025,x+.025,bot+.055,top-.055,-.10,.12)
                    mid=(bot+top)/2
                    for ll,rr in [(l+.056,x-.025),(x+.025,r-.056)]:
                        panel('segmented upper sash meeting rails','trim',ll,rr,mid-.022,mid+.022,-.10,.12)
        ...
        for l,r in (spans if ei in [1,3] else []):panel('segmented low plinth','masonry',l,r,.01,.33,.012,.10)
    ...
    def band(g,m,lo,hi,inner,outer,free_only=False):
        ...
        selected=[1,3] if free_only else list(range(N))
        for i in selected:
            j=(i+1)%N
    ...
    roof_parts.append({'name':'complete four point flat deck','z':z0+H+.008,'full_footprint':True,
                       'low_rim_max_z':z0+H+.29,'rim_edges':[1,3],
    ...
    for (group,mat),(vertices,faces) in groups.items():
        ...
        name='4a Princes Gate Mews | '+group;m=bpy.data.meshes.new(name);m.from_pydata(vertices,[],faces);m.update()
        bm=bmesh.new();bm.from_mesh(m);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(m);bm.free();m.update()
        o=bpy.data.objects.new(name,m);bpy.context.collection.objects.link(o);m.materials.append(materials[mat]);o['building_id']=feature['id'];o['research_object_id']=feature['id']+'::'+group;o['evidence_status']='OSM two-storey four-corner house; exact4a roof unknown; licensed street imagery contextual only';created.append(o.name)
    ...
    return {'created':created,'parameters':{'base_z':z0,'height_m':H,'main_wall_height_m':H,'eaves_height_m':H,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.29,'wall_thickness_m':.40,'opening_count':len(openings)},
            ...
            'evidence_source_ids':['osm-851362839','rbkc-2004-address-list','rbkc-2017-neighbour5','mews-east-2023-context'],
            'observations':['OSM records distinct4a address and two main levels; two UPRNs do not establish a second exterior entry',
                            'Official indexed address list distinguishes4a from4;2017four-storey application concerns5 only',
                            'Actually inspected licensed east-branch photograph shows mews vocabulary, no securely identified4a facade'],
            'uncertainty':['All bay positions, paired casements, door details, pipe and flat roof rim are artistic estimates',
                           'Source height6.849803203m is retained estimated eaves, not measurement',
                           'Both normalized shared segments remain complete and opaque through common heights',
                           'No neighbouring3,4,5,85 roof evidence is transferred; no attic, basement, garage or extra entrance inferred',
                           'Source entry and near-zero datum retained; coordinator supplies finite support to sourceground-.05',
