...
authored=[];results={};input_paths=[BATCH/'reports/context.json',Path(__file__)]
for feature in FEATURES:
    ...
    overrides={'kensington_gore_25':{'masonry':(.76,.74,.67),'trim':(.79,.77,.70)},
               'jay_mews_41_45':{'masonry':(.68,.65,.55),'trim':(.73,.72,.66)},
               'exhibition_road_29':{'masonry':(.40,.19,.12),'trim':(.76,.73,.65),'roof':(.30,.12,.08)}}.get(feature['slug'],{})
    ...
    for o in created:
        o['building_id']=feature['id'];o['research_object_id']=feature['id']+'::batch02::'+o.name
        ...
        o['source_record']='geometry/expansion/batch02/references/'+feature['slug']+'/sources.json'
...
for record in CONTEXT['objects']:
    if record['id'] in IDS|{'way-117417431'} and record['extras'].get('semantic_type')!='entry_support':continue
...
bpy.ops.import_scene.gltf(filepath=str(ROOT/'output/kensington_gore_23_v1/replacement.glb'))
for o in set(bpy.data.objects)-before:o['context_only']=True

scene=bpy.context.scene
scene['scope']='Three replacement buildings with existing neighbourhood context and previous pilot; only batch02 buildings exported'
