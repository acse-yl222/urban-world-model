...
const manifest=JSON.parse(fs.readFileSync('geometry/expansion/batch22/manifest.json'));
const targets=manifest.features.filter(f=>f.id==='way-809386115').map(f=>({id:f.id,entry:f.entry,samples:[0,.2,.5,.8,.9,.95,1,1.2].map(d=>[f.entry.center_xy[0]+f.entry.outward_normal_xy[0]*d,f.entry.center_xy[1]+f.entry.outward_normal_xy[1]*d])}));
const results=[];
