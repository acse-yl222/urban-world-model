"""6 Queens Gate: full mapped ring; individually completed stucco terrace exterior.
...
def build(feature, materials):
    ...
    if len(shared)!=2:raise ValueError('Expected two normalized party-wall segments for no6')
    split=H/levels
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                if not door:
                    ...
                    if ei==entry_edge:entrance=interface
                        ...
                        interface['basis']='HE group portico, speculative number6 front door position';additional_entrances.append(interface)
    ...
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='6 Queens Gate | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
    ...
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'visible_tier_count':5,'tier_height_ratios':[.22,.22,.22,.20,.14],'source_osm_levels':6,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.38},'interfaces':{'entrance':entrance,'additional_entrances':additional_entrances,'additional_supports':additional_supports,'shared_walls':shared,'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-809238786','he-1226082','number2-context-2019','rbkc-basement-2014'],'uncertainty':['Exact number6 facade not confirmed in licensed images; number2 photograph is only same-group context; opening positions artistic','HE five visible storeys includes top attic; sourceOSM6 archived by coordinator, not explained as basement count. Source19.450998m envelope retained and redistributed, not measured','Five tiers include shorter14percent top attic, with complete flat roof and parapet artistic; sourcetotalheight retained, no basement excavation or rooftop plant inferred','Both normalized shared segments on edges0/5 opaque at common height; complete walls preserved; rear wing windows artistic']}
