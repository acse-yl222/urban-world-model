"""25 Princes Gate: mapped five-storey townhouse, exact licensed street evidence and estimated hidden roof.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        count=(3 if ei==6 else max(1,round(L/4.0))) if L>2.8 else 0
        centers=[L*(k+.5)/count for k in range(count)]
        door_info=entry if ei==entry_edge else feature.get('photo_front_entry') if ei==6 else None
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min((1.82 if floor<4 else 1.28) if ei==6 else 1.20,L/count-.8);bot=low+(.30 if floor==1 else .52);top=high-(.76 if floor==1 else .42)
        ...
        if ei==6:
            ...
            for ss in (door_s-1.04,door_s+1.04):
                box('Ionic porch plinth','trim',at(ss,.70,.05),(.44,.44,.10),u,n)
                turned(ss,.70,[(.10,.19),(.20,.17),(3.14,.145),(3.24,.20)],'Ionic porch shafts')
                box('Ionic porch abacus','trim',at(ss,.70,3.30),(.44,.44,.12),u,n)
                # Paired scroll disks, simplified rather than literal sculpture.
                for dx in (-.16,.16):
                    N=16;vv=[at(ss+dx+.10*math.cos(k*2*math.pi/N),d,3.22+.10*math.sin(k*2*math.pi/N)) for d in (.86,.94) for k in range(N)]
                    ff=[tuple(range(N-1,-1,-1)),tuple(range(N,2*N))]+[(k,(k+1)%N,(k+1)%N+N,k+N) for k in range(N)]
                    mesh('Ionic capital volutes','trim',vv,ff)
            panel('Ionic porch entablature','trim',door_s-1.34,door_s+1.34,3.36,balcony-.15,.43,1.18)
            supports.append({'polygon_xy':[list(at(door_s+x,d,0)[:2]) for x,d in [(-1.40,-.12),(1.40,-.12),(1.40,1.12),(-1.40,1.12)]],'top_z':z0,'basis':'Estimated Ionic porch feet support; coordinator owns ground'})
            # Corner quoins and rich fifth-storey cornice, without a sixth tier.
            ...
            for k in range(26):box('fifth-storey cornice brackets','trim',at(.18+k*(L-.36)/25,.15,H-.21),(.11,.25,.20),u,n)
    def offset(d):
        out=[]
    ...
    mesh('complete seven-point concave roof deck','roof',v,f)
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='25 Princes Gate | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='OSM full concave plan; HE1265482 five levels and exact licensed no25 facade; hidden geometry estimated';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.31,'opening_count':len(openings),'wall_thickness_m':.4,'floor_bounds_m':bounds,'front_edge':6},'interfaces':{'entrance':entrance,'additional_entrances':additional,'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-640097055','he-1265482','front23-25-2017'],'uncertainty':['HE1265482 and exact numbered2017photo support five storeys; metric H16.29938m remains inherited estimate','Short fifth tier is included in five levels, no additional attic storey','North front ornament and Ionic porch simplified from exact evidence; north door XY and all dimensions estimated','Full seven-point concave roof is artistically completed behind low parapet; hidden rear facades and roof equipment unverified','All four normalized shared segments retained opaque at overlapping heights, including26 flush attic wall through15.150934m','Original south entrance retained primary; added north photo door and porch support by coordinator; no basement excavation or terrain survey claim']}
