"""46 Princes Gardens: official group identity, licensed terrace-context details.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        elif ei in[1,5]:positions=[]  # Narrow inset return cheeks stay solid.
        else:
    ...
    # The preserved source entry is rear-facing edge2. No front-photo Doric porch
    ...
    for (g,m),(vv,ff)in groups.items():
        if not ff:continue
        name='46 Princes Gardens | '+g;me=bpy.data.meshes.new(name);me.from_pydata(vv,[],ff);me.update()
        bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();me.update()
        ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);me.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Official five-storey group; CC BY-SA2 terrace context, exact46 details and metric heights estimated';created.append(ob.name)
    return {'created':created,'parameters':{'base_z':z0,'main_wall_height_m':H,'height_m':H,'eaves_height_m':H,'levels':5,'roof_levels':0,'roof_max_z':z0+H+.44,'footprint_area_m2':area,'wall_thickness_m':.44,'opening_count':len(openings)},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':[{'name':'full source footprint flat roof','z':z0+H+.008,'surface_separation_m':.008},{'name':'estimated front rear parapet','max_z':z0+H+.44}], 'evidence_source_ids':['osm-640808101','commons-166589109','commons-168502928','westminster-2018-48pg'],'observations':['Official2018Westminster report establishes46–48PrincesGardens group five floors above basement and rear alterations','Licensed close terrace image shows pale stucco, Doric porches, Corinthian-style upper details and balustrades; exact46image-to-bay assignment remains unresolved'],'uncertainty':['Main height16.30095456m and front/rear parapet.44m are estimates, not survey','Three front bays, decorative details and rear openings are estimated from group context, not confirmed same-building photo','Flat roof retained from source; historical group plant-area text does not establish specific46equipment or location, so none invented','Original rear-facing edge2 nearzero entry retained above audited sourceground-.05; no copied front porch, stairs, new front door or basement excavation','All three normalized shared segments retained; upper openings on low-neighbour edge3 use exact height interval','No Blender integration or visual validation claimed by author']}
