...
const counts=Object.fromEntries([...wanted].map(id=>[id,objects.filter(o=>o.id===id).length]));const missing=Object.entries(counts).filter(([,n])=>!n).map(([id])=>id);if(missing.some(id=>id!=='way-359620473'))throw Error('Unexpected missing required assets '+missing.join(','));
