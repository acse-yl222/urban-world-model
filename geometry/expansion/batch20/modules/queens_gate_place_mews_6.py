"""6 Queens Gate Place Mews: full mapped ring; Historical three-storey mews; source main envelope and estimated inset upper tier.
...
def build(feature, materials):
    ...
    materials=dict(materials)  # original warm masonry palette; exact no6 colour remains artistic
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=3 if ei==1 else 2 if ei==3 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.10 if ei==1 else 1.05,L/count-.7)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                if not door:
                    ...
                    # One simple casement mullion split at its meeting rail.
                    mid=(bot+top)/2
                    for lo,hi in [(bot+.07,mid-.027),(mid+.027,top-.07)]:
                        panel('casement mullions','trim',s-.026,s+.026,lo,hi,-.22,.09)
                    panel('casement meeting rail','trim',l+.07,r-.07,mid-.027,mid+.027,-.22,.09)
                else:
    ...
    def closed_slab(rr,lo,hi,name):
        N=len(rr);vs=[(x,y,z0+z) for z in (lo,hi) for x,y in rr]
        vv=[Vector(q) for q in vs[:N]];lookup={tuple(v):i for i,v in enumerate(vv)}
        tris=[tuple(v if isinstance(v,int) else lookup[tuple(v)] for v in tri) for tri in tessellate_polygon([vv])]
        fs=[tuple(reversed(t)) for t in tris]+[tuple(i+N for i in t) for t in tris]
        fs.extend((i,(i+1)%N,(i+1)%N+N,i+N) for i in range(N))
        meshpart(name,'roof',vs,fs)
    # Full mapped lower roof/floor slab remains under the entire inset storey.
    closed_slab(ring,H-.03,H+.02,'full mapped lower roof and upper floor')
    upper_height=float(feature['roof_tier']['height_m'])
    upper_ring=offset(-float(feature['roof_tier']['inset_m']))
    for ei,(a,b) in enumerate(zip(upper_ring,upper_ring[1:]+upper_ring[:1])):
        L=math.dist(a,b);u=((b[0]-a[0])/L,(b[1]-a[1])/L);n=(u[1],-u[0])
        def at(s,d,z):return (a[0]+s*u[0]+d*n[0],a[1]+s*u[1]+d*n[1],z0+z)
        def panel(g,m,l,r,lo,hi,d=-.15,t=.30):block(g,m,at((l+r)/2,d,(lo+hi)/2),(r-l,t,hi-lo),u,n)
        lo,hi=H+.02,H+upper_height
        # Only exposed end elevations get upper windows. Inset long flanks are
        # conservatively blind; mapped lower party walls remain untouched.
        holes=[]
        if ei in (1,3):
            for j in range(2):
                c=L*(j+.5)/2;w=min(1.08,L/2-.65)
                holes.append((c-w/2,c+w/2,H+.57,H+upper_height-.32))
        cuts=sorted({0.,L,*[x for h in holes for x in h[:2]]})
        for l,r in zip(cuts,cuts[1:]):
            h=next((h for h in holes if h[0]<=(l+r)/2<=h[1]),None)
            if h:
                panel('upper pierced masonry','masonry',l,r,lo,h[2]);panel('upper pierced masonry','masonry',l,r,h[3],hi)
            else:panel('upper pierced masonry','masonry',l,r,lo,hi)
        for l,r,bot,top in holes:
            panel('upper recessed glass','glass',l+.06,r-.06,bot+.06,top-.06,-.24,.04)
            for xx in (l+.03,r-.03):panel('upper frame jambs','trim',xx-.03,xx+.03,bot+.06,top-.06,-.18,.08)
            for zz in (bot,top-.06):panel('upper frame rails','trim',l,r,zz,zz+.06,-.18,.08)
            mid=(bot+top)/2
            panel('upper sash rail','trim',l+.06,r-.06,mid-.025,mid+.025,-.18,.08)
            panel('upper projecting sill','trim',l-.06,r+.06,bot-.08,bot-.015,.015,.13)
            openings.append({'edge':ei,'floor':2,'type':'window','width_m':r-l,'height_m':top-bot,'recess_m':.24,'basis':'Historically documented third storey; inset and apertures artistically estimated'})
    closed_slab(upper_ring,H+upper_height,H+upper_height+.08,'closed upper roof slab')
    created=[]
    for (g,m),(vs,fs) in groups.items():
        ...
        name='6 Queens Gate Place Mews | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
        bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(mesh);bm.free()
        ob=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped plan; two source main tiers plus historically supported third tier; heights/setback artistic; exact facade unverified';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'main_wall_height_m':H,'roof_levels':1,'source_main_levels':2,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+upper_height+.08,'visible_tier_count':3,'roof_tier_height_m':upper_height,'roof_tier_inset_m':float(feature['roof_tier']['inset_m'])},'interfaces':{'entrance':entrance,'additional_entrances':[],'additional_supports':[],'shared_walls':shared,'shared_wall_policy':'Every normalized common-height segment retained blind; full footprint preserved'},'openings':openings, 'evidence_source_ids':['osm-810633521','mews2010','rbkc-pp13-02840'],'uncertainty':['Official2013 indexed text describes implemented third storey; fullPDF not viewed; current dimensions unmeasured','Two original source main tiers and full plan preserved; new third tier2.4m high inset.65m explicitly artistic under coordinator contract','No exact licensed no6 elevation found;2010image is street context only','One original west entry retained; no new garage door, basement or unmeasured terrace extent inferred','Both normalized lower shared intervals opaque; upper long sides conservatively blind','Ground-.05 source threshold preserved; coordinator owns finite approach']}
