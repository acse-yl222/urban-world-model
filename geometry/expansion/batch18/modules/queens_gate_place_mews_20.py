"""20 Queen’s Gate Place Mews: mapped two-storey house, licensed streetscape-informed artistic detail.
...
def build(feature, materials):
    ...
    for seg in feature.get('audit',{}).get('normalized_shared_wall_segments',[]):
        ...
        if hi-lo>.001:common.append({**seg,'edge':ei,'s_interval':[lo,hi],'height_interval':zz,'polyline':line,'neighbour':seg['neighbour'],'openings':[],'basis':seg.get('basis','audited shared segment')})
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min((1.08 if floor==0 else 1.72) if ei==entry_edge else 1.24,L/count-.7);bot=low+(.88 if floor==0 else .92);top=high-.60
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'artistic mews house arrangement; exact no.20 not identified in licensed views'})
    ...
    rh=feature['roof_hypothesis'];rise=float(rh['rise_m']);inset=float(rh['inset_m'])
    ...
    mesh('complete four-point roof base deck','roof',v,f)
    # A closed contained mansard surface: adjacent slopes share corner vertices.
    # It is an artistic roof volume, not a confirmed habitable third storey.
    bottom=offset(-.025);upper=offset(-inset);N=len(ring)
    verts=[(x,y,z0+H+.008) for x,y in bottom]+[(x,y,z0+H+rise) for x,y in upper]
    faces=[(i,(i+1)%N,(i+1)%N+N,i+N) for i in range(N)]
    mesh('contained mansard continuous slopes','roof',verts,faces)
    vv=[Vector((x,y,z0+H+rise)) for x,y in upper];vs=[];fs=[]
    for tri in tessellate_polygon([vv]):
        o=len(vs);vs.extend(tuple(vv[q] if isinstance(q,int) else q) for q in tri);fs.append((o,o+1,o+2))
    mesh('mansard complete upper cap','roof',vs,fs)
    roof_parts.append({'type':'complete_polygon_base_cap','z':z0+H+.008,'footprint':ring,'basis':'full mapped closure'})
    roof_parts.append({'type':'contained_mansard_mass','base_ring':bottom,'upper_ring':upper,'rise_m':rise,'inset_m':inset,'max_z':z0+H+rise,'confirmed_habitable_storey':False,'basis':rh['basis']})
    created=[]
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='20 Queen’s Gate Place Mews | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped house with two source-estimated main levels; unresolved second-floor directory clue; artistic contained roof';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+rise,'roof_rise_m':rise,'roof_inset_m':inset,'roof_habitable_storey_confirmed':False,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-810633528','mews2010','bentley-context2024','address-level-zoopla','address-level-192'],'uncertainty':['Original2mainlevels/H6.850305m preserved as inherited estimate, not survey','Two licensed context photos actually inspected but exact20 not identified; facade layout remains artistic and no neighboring garage/showroom identity inferred','Directory addresses mention second floor: unresolved level-count clue, not confirmation of third habitable storey or mansard','Coordinator-approved contained1.8m-rise/.65m-inset mansard mass is artistic roof completion; no dormer or plant asserted','Both shared walls retained; neighbour22 has unknown source height and target-full-height conservative opacity, not measured22height','Original western entry tangent and datum retained; external finite approach from sourceground-.05 owned by coordinator']}
