"""3 Princes Gate Mews OSM-ID-specific two-storey interpretation.
...
def build(feature, materials):
    ...
    # Three upper front sashes and two ground casements flank
    # the retained entry; the unobserved rear has a quieter two-bay treatment.
    floor_positions={4:{0:[2.00,8.20],1:[1.70,5.22,8.74]},
                     1:{0:[1.55,4.58],1:[1.70,4.44]},
                     2:{0:[],1:[2.10]}}
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(floor_positions.get(ei,{}).get(floor,[])):
                ...
                w=(1.75 if ei==4 else 1.25) if floor==0 else 1.18
        ...
        for l,r in (spans if ei in [1,4] else []):panel('segmented low plinth','masonry',l,r,.01,.33,.012,.10)
    ...
    def band(g,m,lo,hi,inner,outer,free_only=False):
        ...
        selected=[1,4] if free_only else list(range(N))
        for i in selected:
            j=(i+1)%N
            faces.extend([(i,j,N+j,N+i),(2*N+i,3*N+i,3*N+j,2*N+j),
                          (i,2*N+i,2*N+j,j),(N+i,N+j,3*N+j,3*N+i)])
            if free_only:
                faces.extend([(i,N+i,3*N+i,2*N+i),(j,2*N+j,3*N+j,N+j)])
        mesh(g,m,v,faces)
    ...
    # Artistic mansard hypothesis after a2003 permission, not proof of its
    # implementation. Full mapped bottom, one continuous sloping ring and inset
    # top close the volume without invented dormers or hidden terrace access.
    upper=offset(-.75);N=len(ring)
    rv=[(x,y,z0+z) for rr,z in [(ring,H+.008),(upper,H+1.15)] for x,y in rr]
    rf=[(i,(i+1)%N,N+(i+1)%N,N+i) for i in range(N)]
    for rr,z,reverse in [(ring,H+.008,True),(upper,H+1.15,False)]:
        vv=[Vector((x,y,z0+z)) for x,y in rr]
        for tri in tessellate_polygon([vv]):
            # Blender Vectors are float32; resolve against the exact Vector
            # inputs, then index original double coordinates in rv. Never
            # compare a returned float32 tuple to an original double tuple.
            local_indices=[q if isinstance(q,int) else vv.index(q) for q in tri]
            base_index=0 if reverse else N
            indices=[base_index+q for q in local_indices]
            rf.append(tuple(reversed(indices)) if reverse else tuple(indices))
    mesh('complete closed estimated mansard','roof',rv,rf)
    roof_parts.append({'name':'complete five point closed estimated mansard',
                       'z':z0+H+.008,'max_z':z0+H+1.15,'top_inset_m':.75,
                       'full_footprint_bottom':True,'dormers':0,
                       'basis':'2003approved mansard motivates artistic hypothesis only; implementation/current form unconfirmed'})
    created=[]
    for (group,mat),(vertices,faces) in groups.items():
        ...
        name='3 Princes Gate Mews | '+group;m=bpy.data.meshes.new(name);m.from_pydata(vertices,[],faces);m.update()
    ...
    return {'created':created,'parameters':{'base_z':z0,'height_m':H,'main_wall_height_m':H,'eaves_height_m':H,'levels':levels,'roof_levels':0,'roof_levels_semantics':'no habitable roof level established; closed roof mass only','roof_max_z':z0+H+1.15,'wall_thickness_m':.40,'opening_count':len(openings)},
            'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,
            'evidence_source_ids':['osm-851362837','mews-east-2023','rbkc-pp0302019'],
            'observations':['Licensed street view shows pale divided windows and sloped slate roofs on unidentified neighbours; exact3 is not identified','Official indexed2003permission approves a mansard addition at3; approval does not establish implementation'],
            'uncertainty':['Facade bays, window dimensions, rear apertures, mansard roof detail and rainwater pipe are artistic completion, not mapped observations',
                           'Two levels are OSM tags;6.849876m is the retained source estimate, not a measured eaves height',
                           'All three normalized shared segments retain walls and suppress overlapping openings; no neighbour walls are removed',
                           'No inferred garage, basement, habitable attic, extra entry or imported1PrincesGateMews ramp',
