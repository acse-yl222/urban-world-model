...
Interactive 3-D web page for the South Kensington core008 urban world model: the three physical fields in
`physics/` (wind, temperature, pollutant concentration; 4 m grid, 768 × 704 cells, north up), the
3-D city model, and the traffic / UAV replay from `agents/demo_rev02/`.

## Layout (restructured 2026-09-14)

| Folder | Content | Size |
|---|---|---|
| `models/` | the packed 3-D models the viewer loads: `south_kensington_core008_web.glb` (254 MB city), `buildings_supplement.glb` (306 added footprints), `south_kensington_current.glb` (station tile, workstation only) | 0.3 GB |
| `assets/` | the same geometry split into one GLB per object, classified (buildings per OSM id, ground, roads, vegetation, street furniture, parked vehicles, simulation layers, actor models); `index.json` catalogue; built by `assets/tools/build_asset_library.mjs` | 1.3 GB |
...
| `physics/` | the physical fields on the shared grid (wind, temperature, sunlight, day cycle, pollution, flood): `.npy` arrays, `manifest.json`, `metadata/`, `figures/`, `README_source_run.md` | 1.4 GB |
| `agents/demo_rev02/` | the colleague's traffic / UAV / bird replay package (data, evidence, modules) | 85 MB |
| `viewer/` | the three.js page (`viewer/3d/`), the `.npy` Range reader (`viewer/npy.js`) | |
| `vendor/` | one copy of three.js 0.185.1 with the fat-line addons; `agents/demo_rev02/vendor` is a symlink to it | 2 MB |
...
fields from `physics/web/` when it exists: one small PNG per frame (65–500 KB; written by
`uv run --with numpy,pillow python3 physics/tools/export_web_frames.py`, ~100 MB for all twelve layers), decoded back to
...
translucent concentration layer at 14 m (log opacity); flooding as a water-depth layer on the ground (`physics/flood/`,
...
Sunlight (`physics/solar/`, added 2026-09-12): the clear-sky solar model (1 m direct-beam shadows from ShadowNet, sky-view
...
3072 × 2816 texture). The scene's sun light follows the frame's real sun position. Day cycle (`physics/temperature3d_solar/`):
...
- Remote viewers get roughly 20 Mbps through the Funnel; the fields play from the pre-rendered PNG frames in `physics/web/`
...
- `physics/` — field data and notes (see `README_source_run.md` and `manifest.json` inside); `solar/` and
