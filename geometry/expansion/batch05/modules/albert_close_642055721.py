"""Albert Close 642055721: mapped anonymous rear wing; artistically completed service facade.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        count=max(1,round(L/2.8)) if L>2.0 else 0
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min(1.55 if floor==0 else 1.12,L/count-.7);bot=low+(.65 if floor==0 else .7);top=high-.52
            ...
            for l,r,bot,top,door,bi in holes:
                s=(l+r)/2;w=r-l
                panel('entrance leaves' if door else 'recessed rear wing windows','door' if door else 'glass',l+.065,r-.065,bot if door else bot+.06,top-.06,-.34,.06)
                ...
                if not door:
                    for frac in (.5,):panel('plain sash mullions','trim',l+w*frac-.02,l+w*frac+.02,bot,top,-.245,.055)
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'artistic plain rear-wing completion; not observed'})
        # Pale stone ground-floor base skirt; above openings sill height.
        if ei!=entry_edge:panel('plain brick plinth','masonry',0,L,.02,.72,.015,.10)
        else:
            panel('plain brick plinth','masonry',0,max(0,door_s-.78),.02,.72,.015,.1);panel('plain brick plinth','masonry',min(L,door_s+.78),L,.02,.72,.015,.1)
    def offset(d):
        out=[]
    ...
    def band(g,m,lo,hi,dep,th):
        ...
        mesh(g,m,v,f)
    for zz,th,dep in [(pitch,.10,.025),(H-.12,.17,.05)]:band('plain eaves and floor course','trim',zz-th/2,zz+th/2,dep,.22)
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='Albert Close 642055721 | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped anonymous rear wing; all architectural detail artistic; neighbourhood material context only';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.47,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-642055721','albert-mansions-context2018'],'uncertainty':['Anonymous OSM building; Albert Close is a geographic working label, not confirmed address or ownership','No permitted photograph confidently identifies this exact asset; all opening arrangements and flat roof details are artistic completion','Original estimated eaves and level count retained; not a measurement','Neighbourhood photograph contributes only muted brick and pale joinery vocabulary, not landmark massing or ornament','Complete concave roof deck and wall perimeter retained; common-height apertures suppressed from audit; no invented plant or large roof structures']}
