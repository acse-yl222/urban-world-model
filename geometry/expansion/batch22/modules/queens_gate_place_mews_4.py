"""4 Queens Gate Place Mews: full mapped ring; Residential mews from OSM; individual facade estimated.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=2 if ei in (0,4,9) else 1 if ei in (2,3,7) else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(.72 if ei==2 else 1.16 if ei==9 else .92,L/count-.65)
    ...
    # wholly inside mapped plan, including all ten corners and the deep concave notch.
    band('inward parapet','trim',H-.10,H+.24,-.15,.24)
    # A watertight full-plan roof slab respects all ten corners and the notch.
    # Original doubles remain authoritative; tessellator vectors only give indices.
    N=len(ring);vs=[(x,y,z0+z) for z in (H-.04,H+.04) for x,y in ring]
    vv=[Vector(v) for v in vs[:N]];lookup={tuple(v):i for i,v in enumerate(vv)}
    ...
    meshpart('closed ten corner roof slab','roof',vs,fs)
    created=[]
    for (g,m),(vs,fs) in groups.items():
        ...
        name='4 Queens Gate Place Mews | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
    ...
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'main_wall_height_m':H,'roof_levels':0,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.24},'interfaces':{'entrance':entrance,'additional_entrances':[],'additional_supports':[],'shared_walls':shared,'shared_wall_policy':'Every normalized common-height segment retained blind; full footprint preserved'},'openings':openings, 'evidence_source_ids':['osm-810633520','mews2010'],'uncertainty':['Exact number4 facade and current use not independently photo verified;4A address does not justify separate geometry or extra entry','Pale masonry, differentiated sash proportions and low flat roof are artistic street-context completion','Two source tiers and original height retained; no transfer of neighbouring6 historical third storey','Full ten-corner plan and concave notch retained with closed slab; all three normalized shared intervals opaque','Original recessed return entryedge4 only; no garage or basement inferred','Ground-.05 source threshold preserved; coordinator owns finite approach']}
