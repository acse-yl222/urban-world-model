"""1 Montrose Court OSM-ID-specific two-storey interpretation.
...
def build(feature, materials):
    ...
    # Modern broad casements; exact bay positions are artistic estimates.
    floor_positions={0:{0:[2.15,6.25,9.70],1:[2.15,6.25,9.70]},
                     2:{0:[2.20,10.95],1:[2.20,6.65,10.95]}}
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(floor_positions.get(ei,{}).get(floor,[])):
                ...
                w=(2.20 if ei==0 else 1.85) if floor==0 else (2.20 if ei==0 else 1.90)
                bot=low+(.85 if floor==0 else .85);top=high-.70
    ...
    mesh('complete original five point roof deck','roof',[(x,y,z0+H+.008) for x,y in ring],ff)
    ...
    roof_parts.append({'name':'complete five point flat deck','z':z0+H+.008,'full_footprint':True,
    ...
    return {'created':created,'parameters':{'base_z':z0,'height_m':H,'main_wall_height_m':H,'eaves_height_m':H,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.29,'wall_thickness_m':.40,'opening_count':len(openings)},
            ...
            'evidence_source_ids':['osm-640457472','court-identity','westminster-2014-item9','architect-estate-context','clarke-context-2010'],
            'observations':['Judgment identifies number1 as northernmost two-storey house east of service road, distinct from large block',
                            'Exact-address2014approval concerns two-storey extension and entrance relocation, not proof of implementation',
                            'Unnumbered architect project describes modern1950s estate terrace, not exact1 facade'],
            'uncertainty':['Window bays, modern casement details, plain entrance leaf, pipe and flat roof rim are artistic estimates',
                           'Source height6.849992113m is retained estimated eaves, not measurement',
                           'All three normalized shared segments remain complete and opaque at common heights',
                           'No extra side extension, historical relocated door, attic or basement geometry added',
