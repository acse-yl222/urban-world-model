...
const counts=Object.fromEntries([...wanted].map(id=>[id,objects.filter(o=>o.id===id).length]));const missing=Object.entries(counts).filter(([,n])=>!n).map(([id])=>id);if(missing.some(id=>!['way-359620473','way-158555530'].includes(id)))throw Error('Unexpected missing required assets '+missing.join(','));
const basis={method:'Decoded original source plus latest delivered replacement overlays in chronological report order; preserved original entry support meshes',inputs,replacements,required_building_object_counts:counts,missing_ids:missing,missing_context_policy:"Batch15 missing way-158555530 audited absent from asset index and original GLB node metadata; no parent memberships. OSM ten levels do not establish scene wall height; preserve exact mapped shared boundary, conservatively opaque target wall, no surrogate neighbour geometry. See missing_neighbour_audit.json"};
