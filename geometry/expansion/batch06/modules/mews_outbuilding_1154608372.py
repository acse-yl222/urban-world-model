...
def build(feature, materials):
    ...
    band('low mitred coping','trim',H+.16,H+.205,-.27,-.005)
    ...
    for (group,mat),(vs,fs) in groups.items():
        name='Mews outbuilding1154608372 | '+group;me=bpy.data.meshes.new(name);me.from_pydata(vs,[],fs);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[mat]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+group;ob['evidence_status']='Refined ancillary shell; mapped one floor, unknown openings, estimated heights';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':1,'opening_count':0,'wall_thickness_m':.28,'roof_max_z':z0+H+.205,'small_solid_outbuilding':True,'refinement_class':'refined ancillary shell'},'interfaces':{'entrance':None,'entrance_status':'No source entry; opening locations unknown, none invented','shared_walls':[],'shared_wall_policy':'Full mapped five-point shell retained; no geometry correction to hide GIS overlap','gis_overlap_uncertainty':feature.get('audit',{}).get('positive_area_overlaps',[])},'openings':[],'evidence_source_ids':['osm-1154608372','outbuilding8372-context'],'uncertainty':['Exact building use and entrances unknown; no source door/window mesh and no confirmed target photograph','Inherited footprint overlaps Huxley relation37787 by0.05570968m2 (0.2328%); centimetre-scale GIS uncertainty retained, not clipped/moved','OSM one floor retained; source eaves estimated, not measured','Coping, thin slab and masonry trim are artistic weatherproofing details; this is refined ancillary shell, not survey-accurate facade']}
