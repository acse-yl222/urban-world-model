"""85 Princes Gate Mews OSM-ID-specific two-storey interpretation.
...
def build(feature, materials):
    ...
    floor_positions={0:{0:[2.0,5.25,11.25],1:[1.72,4.98,8.24,11.50]},
                     1:{0:[2.00,5.95],1:[2.05,5.90]},
                     2:{0:[],1:[2.10,6.40]},3:{0:[],1:[]}}
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(floor_positions.get(ei,{}).get(floor,[])):
                ...
                w=(1.65 if ei==0 else 1.20) if floor==0 else 1.18
        ...
        for l,r in (spans if ei in [0,1] else []):panel('segmented low plinth','masonry',l,r,.01,.33,.012,.10)
    ...
    def band(g,m,lo,hi,inner,outer,free_only=False):
        ...
        selected=[0,1] if free_only else list(range(N))
        for i in selected:
            j=(i+1)%N
    ...
    # Historical three storeys motivate an inset upper tier, not a claim that
    # the refused side/rear dormer was implemented. Main shared walls stop at H.
    ...
    for ei,(a,b) in enumerate(zip(upper,upper[1:]+upper[:1])):
        ...
        positions=[L*f for f in [.18,.50,.82]] if ei==0 else [L*.28,L*.72] if ei==1 else []
    ...
    roof_parts.extend([{'name':'full original six point footprint deck','z':z0+H+.008,'full_footprint':True},
                       {'name':'estimated inset historical third tier','count':1,'inset_m':inset,'rise_m':rise,
                        'max_z':z0+H+rise,'glazing':upper_openings,'boundary_shared_height_increased':False,
                        'basis':'Historical3storey wording interpreted artistically; no claim approved dormer built'}])
    created=[]
    for (group,mat),(vertices,faces) in groups.items():
        ...
        name='85 Princes Gate Mews | '+group;m=bpy.data.meshes.new(name);m.from_pydata(vertices,[],faces);m.update()
    ...
    return {'created':created,'parameters':{'base_z':z0,'height_m':H,'main_wall_height_m':H,'eaves_height_m':H,'levels':levels,'roof_levels':1,'authored_total_visible_tiers':3,'roof_levels_semantics':'Estimated inset third tier motivated by historical text; not surveyed','roof_max_z':z0+H+rise,'wall_thickness_m':.40,'opening_count':len(openings),'inset_upper_opening_count':len(upper_openings),'total_authored_apertures':len(openings)+len(upper_openings)},
            'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,
            'evidence_source_ids':['osm-851362852','mews-east-2023','judgment-2015-ewhc2458','rbkc-pp1406202'],
            'observations':['Licensed street view shows pale divided windows and sloped slate roofs on unidentified neighbours; exact85 is not identified','Historical2015judgment describes85as3storey before proposed basement; exact upper roof shape unknown'],
            'uncertainty':['Facade bays, window dimensions, rear apertures, inset roof-tier detail and rainwater pipe are artistic completion, not mapped observations',
                           'Two main levels retained fromOSM;historical3storey wording informs inset third tier;6.849712875m is the retained source estimate, not a measured eaves height',
