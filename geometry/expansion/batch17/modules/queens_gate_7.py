"""7 Queens Gate: full mapped ring; individually completed stucco terrace exterior.
...
def build(feature, materials):
    ...
    if len(shared)!=2:raise ValueError('Expected two normalized party-wall segments for no7')
    split=H/levels
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=3 if ei==6 else 2 if ei in (2,4) else 1 if ei in (1,3) else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min((1.42 if level in (1,2) else 1.16) if ei==6 else 1.0,L/count-.8)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                for xx in [l+.035,r-.035]:panel('painted joinery','trim',xx-.035,xx+.035,bot+(0 if door else .07),(top-(r-l)/2 if ei==6 and level==3 and not door else top-.07),-.22,.09)
                ...
                if not door:
                    ...
                    if ei==entry_edge:entrance=interface
                        ...
                        interface['basis']='HE group portico, speculative number7 front door position';additional_entrances.append(interface)
        ...
        if ei==6:
            ...
            es=entry_s
            # Official grouped Doric portico, fitted to the original no7 doorway.
            ...
            for ss in [es-.84,es+.84]:
                panel('Doric column square plinths','trim',ss-.17,ss+.17,0,.17,.72,.34)
                profile=[(.17,.132),(.24,.120),(1.35,.115),(2.49,.102),(2.54,.102),(2.64,.155)]
                vs=[];fs=[];N=24
                for zz,rad in profile:
                    for j in range(N):
                        q=2*math.pi*j/N;vs.append(at(ss+rad*math.cos(q),.72+rad*math.sin(q),zz))
                ...
                panel('Doric column abaci','trim',ss-.18,ss+.18,2.64,2.78,.72,.36)
            panel('projecting portico architrave','trim',es-1.15,es+1.15,2.78,2.94,.43,1.02)
            panel('projecting portico cornice','trim',es-1.20,es+1.20,2.94,3.04,.45,1.04)
            support={'polygon_xy':[list(at(es+x,d,0)[:2]) for x,d in [(-1.20,-.08),(1.20,-.08),(1.20,.98),(-1.20,.98)]],'top_z':z0,'basis':'Estimated freestanding Doric column feet support at original entry; coordinator owns closed finite slab; no new entrance'}
    ...
    vv=[Vector((x,y,z0+H-.07)) for x,y in ring];vs=[];fs=[]
    for tri in tessellate_polygon([vv]):
        off=len(vs);vs.extend(tuple(vv[v] if isinstance(v,int) else v) for v in tri);fs.append((off,off+1,off+2))
    meshpart('complete mapped roof basecap','roof',vs,fs)
    created=[]
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='7 Queens Gate | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
