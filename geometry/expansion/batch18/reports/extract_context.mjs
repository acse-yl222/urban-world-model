...
const counts=Object.fromEntries([...wanted].map(id=>[id,objects.filter(o=>o.id===id).length]));const missing=Object.entries(counts).filter(([,n])=>!n).map(([id])=>id);if(missing.some(id=>!['way-359620473','way-810633529'].includes(id)))throw Error('Unexpected missing required assets '+missing.join(','));
const basis={method:'Decoded original source plus latest delivered replacement overlays in chronological report order; preserved original entry support meshes',inputs,replacements,required_building_object_counts:counts,missing_ids:missing,missing_context_policy:"way-810633529 audited absent from original and supplemental GLB metadata; see missing_neighbour_audit.json. way-359620473 absent from source asset index, source GLB decode and source-part aliases; retain mapped neighbour with unknown height, no surrogate geometry"};
