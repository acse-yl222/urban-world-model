"""Albert Close 642055722: full mapped ring; individually completed terraced mews exterior.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=max(1,int(L/3.25)) if L>2.5 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.25 if level==0 else 1.15,L/count-.8)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                panel('brick header lintels','masonry',l-.12,r+.12,top+.015,top+.20,.022,.13)
                panel('slim pale lintel accent','trim',l-.04,r+.04,top+.20,top+.255,.035,.14)
                if not door:
    ...
    meshpart('complete mapped roof membrane','roof',vs,fs)
    # A small inward shed roof differs from the neighbouring hip roof.
    # Its exact form is artistic; the complete polygon deck below remains intact.
    c=(800.75,40.8);u=(-.1438792552457349,.9895952505494016);n=(u[1],-u[0])
    def roofpoint(s,t,z):return (c[0]+s*u[0]+t*n[0],c[1]+s*u[1]+t*n[1],z0+z)
    footprint=[(-3.6,-2.4),(3.6,-2.4),(3.6,2.4),(-3.6,2.4)]
    points=[roofpoint(ss,tt,H-.04) for ss,tt in footprint]+[roofpoint(ss,tt,H+.22+(ss+3.6)/7.2*.7) for ss,tt in footprint]
    meshpart('estimated internal shed roof curb','masonry',points,[(0,3,2,1),(0,1,5,4),(1,2,6,5),(2,3,7,6),(3,0,4,7)])
    meshpart('estimated shallow shed roof','roof',points[4:],[(0,1,2,3)])
    created=[]
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='Albert Close 642055722 | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
        bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(mesh);bm.free()
        ob=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped footprint; inherited estimated four-floor massing; artistic red-brick rear-wing completion';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.92},'interfaces':{'entrance':entrance,'shared_walls':shared,'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-642055722','albert722-context2018'],'uncertainty':['Exact way-642055722 facade not confirmed in licensed images; all bay positions and window sizes artistically completed','Current address and institutional ownership not independently confirmed; no signage modelled','Small internal shed roof and door proportions artistically completed; full original footprint deck and eaves retained; no roof equipment inferred','Shared-wall adjacency unknown; full walls preserved']}
