...
const v=JSON.parse(fs.readFileSync('geometry/expansion/batch08/reports/museum_lane_passage_interface.json')).interface;const w=v.west_wall_xy,e=v.east_wall_xy,d=v.west_to_east_direction_xy;const xy=[w.map((n,i)=>n-.9*d[i]),w,v.axis_center_xy,e,e.map((n,i)=>n+.9*d[i])];const labels=['west_outside_0.9m','west_wall','center','east_wall','east_outside_0.9m'];const targets=xy.map((p,i)=>({id:labels[i],entry:{center_xy:p},samples:[p]}));
const results=[];
...
fs.writeFileSync('geometry/expansion/batch08/reports/passage_ground_audit.json',JSON.stringify({source:'south_kensington_core008_web.glb',method:'Decoded original mesh; vertical XY barycentric hits at west, center, east and .9m outside both ends. Bounds distances only for nearby no-hit objects, not exact mesh distances.',targets,objects:results},null,2));console.log(JSON.stringify(results.filter(o=>o.hits.length).map(o=>({name:o.name,hits:o.hits})),null,2));
