"""16 Queen’s Gate Place Mews: mapped two-storey house, licensed streetscape-informed artistic detail.
...
def build(feature, materials):
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min(1.42 if floor==0 else 1.18,L/count-.7);bot=low+(.82 if floor==0 else .90);top=high-.60
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'artistic mews house arrangement; exact no.16 not identified in licensed views'})
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='16 Queen’s Gate Place Mews | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='OSM-addressed two-storey house; exact-photo identification unresolved; streetscape-informed artistic detail';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.31,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-810633526','mews2010','bentley-context2024'],'uncertainty':['OSM address and two levels retained; inherited 6.850323m eaves is a formula estimate, not survey','Both licensed photographs were actually inspected but neither reliably identifies no.16; muted masonry and painted sash joinery are context-derived; opening layout and flat roof are artistic','No nearby mansard, garage or motor showroom identity is asserted for this property; unrelated 16 Queens Gate Mews planning result excluded','Complete five-point roof and walls retained; all three normalized shared segments kept opaque at supplied height intervals, including short relation contact','Original eastern entrance lateral position and base retained; external ground and approximately 50mm ground-to-door difference belong to coordinator']}
