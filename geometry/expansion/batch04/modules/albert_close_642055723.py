"""Albert Close 642055723: full mapped ring; individually completed terraced mews exterior.
...
def build(feature, materials):
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=max(1,int(L/2.95)) if L>2.5 else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min(1.1 if level==0 else 1.0,L/count-.8)
            ...
            # Separate roof-adjacency exclusion above 84's 6.899m main wall.
            if ei==2:holes=[h for h in holes if h[4] or z0+h[2]>8.55]
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                if not door:
                    ...
                    if level>0:
                        panel('upper sash meeting rail','trim',l,r,(bot+top)/2-.04,(bot+top)/2+.04,-.19,.06)
                else:
    ...
    meshpart('complete concave roof membrane','roof',vs,fs)
    # Modest artistic hip roof entirely inside central rear-wing plan.
    # Full mapped cap remains below; no landmark gables or dormers inferred.
    c=(804.4,24.0);u=(-.26368630374840607,.9646084870119603);n=(u[1],-u[0])
    def roofpoint(s,t,z):return (c[0]+s*u[0]+t*n[0],c[1]+s*u[1]+t*n[1],z0+z)
    block('estimated hip roof curb','masonry',(c[0],c[1],z0+H+.07),(12.,4.,.22),u,n)
    corners=[roofpoint(-6,-2,H+.18),roofpoint(6,-2,H+.18),roofpoint(6,2,H+.18),roofpoint(-6,2,H+.18)]
    corners += [roofpoint(-4,0,H+1.25),roofpoint(4,0,H+1.25)]
    meshpart('estimated shallow hip roof','roof',corners,[(0,1,5,4),(1,2,5),(2,3,4,5),(3,0,4)])
    created=[]
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='Albert Close 642055723 | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
        bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(mesh);bm.free()
        ob=bpy.data.objects.new(name,mesh);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Mapped footprint; inherited estimated four-floor massing; artistic plain rear-wing facade completion';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+1.25},'interfaces':{'entrance':entrance,'shared_walls':shared,'roof_adjacency_exclusions':[{'edge':2,'neighbour':'way-492431542','new_opening_bottom_min_z':8.55,'basis':'84 saltbox ridge8.4992868 plus51mm clearance; this is NOT shared wall height'}],'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-642055723','albert-mansions-context2018'],'uncertainty':['Exact way-642055723 facade not confirmed in licensed images; all bay positions and window sizes artistically completed','Current address and institutional ownership not independently confirmed; no signage modelled','Shallow interior hip roof is artistic addition; exact original eaves retained and complete mapped base cap beneath; no plant inferred','Shared-wall adjacency unknown; full walls preserved']}
