"""Museum service infill 850528335: mapped anonymous mapped infill between Science Museum and Natural History Museum; artistically completed elevations.
...
def build(feature, materials):
    ...
    for adj in feature.get('audit',{}).get('adjacent_or_detailed_neighbours_within_35m',[]):
        ...
        uncertain_height=not zz
        if uncertain_height:zz=[z0,z0+H]
        ...
        for line in lines:
            for p,q in zip(line,line[1:]):
                for ei,(a,L,u,n) in enumerate(edges):
                    ...
                    if hi-lo>.03:common.append({'edge':ei,'s_interval':[lo,hi],'height_interval':zz,'height_basis':'conservative whole-target aperture exclusion; neighbour eaves unresolved' if uncertain_height else 'audited shared wall interval','observed_height_interval':None if uncertain_height else zz,'polyline':[p,q],'neighbour':adj['id'],'openings':[]})
    ...
    for ei,(a,L,u,n) in enumerate(edges):
        ...
        for floor in range(levels):
            ...
            for bi,s in enumerate(centers):
                w=min(1.30,L/count-.85);bot=low+1.12;top=high-.68
            ...
            for l,r in zip(cuts,cuts[1:]):
                ...
                if h:panel('pierced service wing masonry','masonry',l,r,low,h[2]);panel('pierced service wing masonry','masonry',l,r,h[3],high)
                else:panel('pierced service wing masonry','masonry',l,r,low,high)
            for l,r,bot,top,door,bi in holes:
                s=(l+r)/2;w=r-l
                panel('entrance leaves' if door else 'recessed service wing casements','door' if door else 'glass',l+.065,r-.065,bot if door else bot+.06,top-.06,-.34,.06)
                ...
                if not door:
                    for frac in (.5,):panel('paired casement mullions','metal',l+w*frac-.02,l+w*frac+.02,bot,top,-.245,.055)
                    for frac in (.76,):panel('casement toplight transoms','metal',l,r,bot+(top-bot)*frac-.021,bot+(top-bot)*frac+.021,-.245,.055)
                ...
                openings.append({'edge':ei,'floor':floor,'bay':bi,'type':'door' if door else 'window','width_m':w,'height_m':top-bot,'recess_m':.34,'basis':'artistic service wing completion; direct facade unconfirmed'})
    ...
    def band(g,m,lo,hi,dep,th):
        ...
        mesh(g,m,v,f)
    for zz,th,dep in [(H-.12,.17,-.15)]:band('plain eaves and floor course','trim',zz-th/2,zz+th/2,dep,.22)
    band('service wing parapet','masonry',H,H+.37,-.17,.34);band('stone parapet coping','trim',H+.37,H+.47,-.20,.4)
    ...
    for (g,m),(v,f) in groups.items():
        if not f:continue
        name='Museum service infill 850528335 | '+g;me=bpy.data.meshes.new(name);me.from_pydata(v,[],f);me.update();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));bm.to_mesh(me);bm.free();ob=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(ob);ob.data.materials.append(materials[m]);ob['building_id']=feature['id'];ob['research_object_id']=feature['id']+'::'+g;ob['evidence_status']='Anonymous museum-area infill; four levels are source estimate; exact current target appearance unresolved';created.append(ob.name)
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'base_z':z0,'levels':levels,'roof_levels':0,'roof_max_z':z0+H+.47,'opening_count':len(openings),'wall_thickness_m':.4},'interfaces':{'entrance':entrance,'additional_entrances':[],'shared_walls':common},'openings':openings,'roof_parts':roof_parts,'evidence_source_ids':['osm-850528335','commons-museums-aerial-2011','commons-museum-lane-2012'],'uncertainty':['Working museum service infill label is not confirmed ownership, address, function or campus affiliation','Four levels and retained 13.1506579842m eaves are original procedural estimates; current exact target is not resolved in available permitted photos','Museum Lane photo shows a low ornamental gateway east of the mapped target; it is not assigned to this 35m-long strip and cannot establish target height','Shared edges0/1/2/3 to Science Museum and Natural History Museum remain complete and conservatively opaque across target height; neighbour actual eaves unresolved','Only exposed edge4 receives an inherited entry and three modest upper casements; all facade details and complete flat roof/parapet are artistic completion']}
