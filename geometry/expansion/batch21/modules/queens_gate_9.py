"""9 Queens Gate: full mapped ring; individually completed stucco terrace exterior.
...
def build(feature, materials):
    ...
    if len(shared)!=3:raise ValueError('Expected three normalized party-wall segments for no9')
    split=H/levels
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        count=3 if ei==5 else 2 if ei==3 else 1 if ei in (1,2) else 0
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for interface in shared:
                if interface['edge']==ei:
                    lo,hi=interface.get('height_interval') or interface['operational_opaque_height_interval']
    ...
    for (g,m),(vs,fs) in groups.items():
        ...
        name='9 Queens Gate | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
    ...
    return {'created':created,'parameters':{'height_m':H,'eaves_height_m':H,'main_wall_height_m':H,'base_z':z0,'levels':levels,'visible_tier_count':5,'tier_height_ratios':[.22,.22,.22,.20,.14],'source_osm_levels':6,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.38},'interfaces':{'entrance':entrance,'additional_entrances':additional_entrances,'additional_supports':additional_supports,'shared_walls':shared,'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-809238789','he-1226082','number2-context-2019'],'uncertainty':['Exact number9 facade not confirmed in licensed images; number2 is only group context; retained rear entry and approved estimated additional street door/Doric portico are unverified in real appearance','HE five visible storeys includes top attic; sourceOSM6 archived by coordinator, not explained as basement count. Source19.451097m envelope retained and redistributed, not measured','Five tiers include shorter14percent top attic, with complete flat roof and parapet artistic; sourcetotalheight retained, no basement excavation or rooftop plant inferred','Three normalized segments on edges0/1/4 retained; neighbour790 height null preserved and full-target operational mask is precaution only, not source height; rear windows artistic']}
