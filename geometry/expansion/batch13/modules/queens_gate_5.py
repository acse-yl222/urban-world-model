"""5 Queens Gate: full mapped ring; individually completed stucco terrace exterior.
...
def build(feature, materials):
    ...
    if len(shared)!=2:raise ValueError('Expected two normalized party-wall segments for no5')
    split=H/levels
    ...
    for ei,(a,b,L,u,n) in enumerate(edges):
        ...
        for level,(low,high) in enumerate(zip(floor_bounds,floor_bounds[1:])):
            ...
            for bi,s in enumerate(centers):
                w=min((1.40 if level in (1,2) else 1.18) if ei==6 else 1.0,L/count-.8)
            ...
            for l,r,bot,top,door,bi in holes:
                ...
                if not door:
                    ...
                    if ei==entry_edge:entrance=interface
                        ...
                        interface['basis']='HE group portico, speculative number5 front door position';additional_entrances.append(interface)
        ...
        if ei==6:
            ...
            for ss in [L/6,L/2,5*L/6]:
                for xx in [ss-.52,ss+.52]:
                    panel('individual window consoles','trim',xx-.065,xx+.065,3*split-.26,3*split-.05,.09,.20)
    def offset(d):
        out=[]
    ...
    for (g,m),(vs,fs) in groups.items():
        if not fs:continue
        name='5 Queens Gate | '+g;mesh=bpy.data.meshes.new(name);mesh.from_pydata(vs,[],fs);mesh.update()
    ...
    return {'created':created,'parameters':{'height_m':H,'base_z':z0,'levels':levels,'opening_count':len(openings),'wall_thickness_m':.36,'roof_max_z':z0+H+.38},'interfaces':{'entrance':entrance,'additional_entrances':additional_entrances,'additional_supports':additional_supports,'shared_walls':shared,'shared_wall_policy':'All mapped boundary walls retained; no unverified party-wall deletions'},'openings':openings,'evidence_source_ids':['osm-809238785','he-1226082','number2-context-2019'],'uncertainty':['Exact number5 facade not confirmed in licensed images; number2 photograph is only same-group context; opening positions artistic','OSM address5 retained; exact current facade not independently identified; HE group five storeys conflicts with OSM six and no metric correction is justified','Flat roof, parapet and door proportions artistically completed; existing eaves height retained exactly; no roof plant inferred','Both normalized shared segments on edges0/5 opaque at common height; complete walls preserved; rear wing windows artistic']}
