...
for(let k=0;k<N;k++){const u=frames[a*N+k]*(1-mix)+frames[b*N+k]*mix,q=Number.isFinite(u)?Math.max(0,Math.min(4,u/5)):0,c=Math.min(3,Math.floor(q)),f=q-c;for(let z=0;z<3;z++)pixels[k*4+z]=palette[c][z]*(1-f)+palette[c+1][z]*f;pixels[k*4+3]=Number.isFinite(u)?255:0;}
const rpm=rotorSystem.update(t,document.querySelector('#spin').checked);texture.needsUpdate=true;mat.opacity=opacity;field.visible=opacity>0;renderer.render(scene,camera);ctx.drawImage(renderer.domElement,0,0);
...
ctx.fillStyle='#8fe3d2';ctx.font='600 17px system-ui';ctx.fillText('WIND FARM / 23 TURBINES',64,58);ctx.fillStyle='#f1f7fc';ctx.font='600 38px system-ui';ctx.fillText(chapter,64,110);ctx.fillStyle='#b5c9d6';ctx.font='20px system-ui';ctx.fillText('2 m simulation grid   ·   Terrain-following slice: 80 m AGL   ·   Actuator disks',64,151);
ctx.textAlign='right';ctx.fillStyle='#f1f7fc';ctx.font='600 37px system-ui';ctx.fillText(`${t.toFixed(1)} / 300 s`,1856,77);ctx.font='18px system-ui';ctx.fillStyle='#b5c9d6';ctx.fillText('12,206 solver steps · 151 saved frames',1856,109);ctx.fillText(`Visual rotor speeds: ${rpm[0].toFixed(1)}–${rpm[1].toFixed(1)} rpm (assumed)`,1856,141);ctx.textAlign='left';
ctx.fillStyle='#101923d9';ctx.fillRect(48,hh-120,1824,100);ctx.font='16px system-ui';ctx.fillStyle='#b5c9d6';ctx.fillText('Independent MAC · Visual rotor motion: assumed TSR 7 · Display sampled at 8 m',64,hh-45);
...
document.querySelector('#clock').textContent=t.toFixed(1)+' s';document.querySelector('#time').value=t;
}
window.movie={ready:true,meta,rotorSystem,frame(f,n){const k=f/(n-1),p=Math.max(0,Math.min(1,(k-.12)/.76)),t=300*p;const angle=.18+Math.sin(k*Math.PI)*.20,dist=4500-k*600;camera.position.set(200+Math.sin(angle)*dist,2600-k*550,Math.cos(angle)*dist);controls.target.set(200,230,0);controls.update();render(t,Math.min(.72,Math.max(0,(k-.08)*12)),k<.12?'TERRAIN + TURBINE GEOMETRY':'SIMULATED WIND + GEOMETRY');return canvas.toDataURL('image/jpeg',.94).split(',')[1];},render};
function animate(now){const dt=last?(now-last)/1000:0;last=now;if(playing)time=(time+dt*12.5)%300;controls.update();render(time);requestAnimationFrame(animate);}if(!new URLSearchParams(location.search).has('capture'))requestAnimationFrame(animate);else render(0);
